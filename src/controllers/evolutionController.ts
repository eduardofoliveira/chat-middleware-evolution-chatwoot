...
const handleEvolutionWebhook = async (
  ...
  try {
    ...
    let contact = await findContactIdentifier({ identifier: data.key.remoteJid, account_id: Number(account_id) });
    if (!contact && !isGroup) {
      contact = await findContactPhoneNumber({ phone_number: `+${/(.*)@/.exec(data.key.remoteJid)?.[1] || data.key.remoteJid}`, account_id: Number(account_id) });
      if (!contact) {
        await createContact({
          identifier: data.key.remoteJid,
          ...
          country_code: /(.*)@/.exec(data.key.remoteJid)?.[1]?.startsWith('55') ? 'Brazil' : '',
          custom_attributes: JSON.stringify({ source: 'evolution' }),
          additional_attributes: JSON.stringify({
            city: "",
            country: /(.*)@/.exec(data.key.remoteJid)?.[1]?.startsWith('55') ? 'Brazil' : '',
            description: "",
            company_name: "",
            country_code: /(.*)@/.exec(data.key.remoteJid)?.[1]?.startsWith('55') ? 'BR' : '',
            social_profiles: {
              github: "",
              twitter: "",
              facebook: "",
        ...
        contact = await findContactIdentifier({ identifier: data.key.remoteJid, account_id: Number(account_id) });

        const profileData = await axios.post(`${EVOLUTION_URL}/chat/fetchProfile/${body.instance}`, {
      ...
      const groupInfo = await axios.get(`${EVOLUTION_URL}/group/findGroupInfos/${body.instance}`, {
        params: {
    ...
    if (data.messageType === "albumMessage") {
      ...
      setTimeout(async () => {
        ...
        const form = new FormData();
        if (albumCaptions[albumId]) form.append("content", `${header}${albumCaptions[albumId]}`);
        if (albums[albumId]) {
    ...
    if (["imageMessage", "videoMessage"].includes(data.messageType) && data.message?.messageContextInfo?.messageAssociation?.associationType === 'MEDIA_ALBUM') {
      const parentId =
      ...
      await salvarMidia(
        ...
        path.resolve('files', `${data.key.id}.${mime.extension(data.message?.imageMessage?.mimetype || data.message?.videoMessage?.mimetype || 'bin')}`)
      )
      ...
      return reply.code(200).send({ status: 'album_image_processed', parentId });
    }
    ...
    if (data.messageType === 'imageMessage') {
      // mensagem de imagem
      await salvarMidia(data.message.imageMessage, path.resolve('files', `${data.key.id}.${mime.extension(data.message.imageMessage.mimetype || 'jpg')}`));
      const form = new FormData();
      form.append("attachments[]", fs.createReadStream(path.resolve('files', `${data.key.id}.${mime.extension(data.message.imageMessage.mimetype || 'jpg')}`)));
      form.append("message_type", 'incoming');
      ...
      fs.unlink(
        path.resolve('files', `${data.key.id}.${mime.extension(data.message.imageMessage.mimetype || 'jpg')}`), (err) => {
          if (err) {
    ...
    if (data.messageType === 'videoMessage') {
      // mensagem de vídeo
      await salvarMidia(data.message.videoMessage, path.resolve('files', `${data.key.id}.${mime.extension(data.message.videoMessage.mimetype || 'mp4')}`));
      const form = new FormData();
      form.append("attachments[]", fs.createReadStream(path.resolve('files', `${data.key.id}.${mime.extension(data.message.videoMessage.mimetype || 'mp4')}`)));
      form.append("content", header);
      form.append("message_type", 'incoming');
      ...
      fs.unlink(
        path.resolve('files', `${data.key.id}.${mime.extension(data.message.videoMessage.mimetype || 'mp4')}`), (err) => {
          if (err) {
    ...
    if (data.messageType === 'stickerMessage') {
      ...
      await salvarMidia(data.message.stickerMessage, path.resolve('files', `${data.key.id}.webp`));
      const form = new FormData();
      form.append("attachments[]", fs.createReadStream(path.resolve('files', `${data.key.id}.webp`)));
      form.append("content", header);
      form.append("message_type", 'incoming');
    ...
    if (data.messageType === 'audioMessage') {
      // mensagem de áudio
      await salvarMidia(data.message.audioMessage, path.resolve('files', `${data.key.id}.ogg`));
      const form = new FormData();
      form.append("attachments[]", fs.createReadStream(path.resolve('files', `${data.key.id}.ogg`)));
      form.append("content", header);
      form.append("message_type", 'incoming');
    ...
    if (data.messageType === 'documentMessage') {
      ...
      await salvarMidia(data.message.documentMessage, path.resolve('files', `${data.key.id}.${data.message.documentMessage.fileName?.split('.').pop() || 'bin'}`));
      const caption = data.message.documentMessage.caption ? `${data.message.documentMessage.caption}\n` : '';

      const form = new FormData();
      form.append("attachments[]", fs.createReadStream(path.resolve('files', `${data.key.id}.${data.message.documentMessage.fileName?.split('.').pop() || 'bin'}`)));
      form.append("content", `${header}${caption}📎 Documento: ${data.message.documentMessage.fileName}`);
      form.append("message_type", 'incoming');
